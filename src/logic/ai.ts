...
export class SimpleAI {
    static makeMove(pieces: PieceType[], color: PieceColor, boardSize: number): { updatedPieces: PieceType[] } {
        ...
        for (const myPiece of myPieces) {
            ...
            if (attackTarget) {
                const updatedPieces = pieces
                    .map((p) => (p.id === myPiece.id ? { ...p, movedThisTurn: true } : p))
        ...
        for (const myPiece of myPieces) {
            const possibleMoves = reachableCells(myPiece, pieces, boardSize, 5).filter(
    ...
    private static findAttackTarget(myPiece: PieceType, enemyPieces: PieceType[]): PieceType | null {
